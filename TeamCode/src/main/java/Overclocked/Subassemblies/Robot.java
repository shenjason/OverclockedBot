...
public class Robot {
    ...
    public final double HEADING_SENSITIVITY = 0.4;
...
//    public CameraDetection cameraDetection;
    ...
    private Timer transferTimer, modeSwitchTimer, autoAngleTimer, autoTuneCooldown, p2pPathTimer, releaseOuttakeTimer;
    private boolean inTransfer1, inTransfer2, switching, SIDE, MODE, driveLocked, autoTune, autoPickUp, inP2P, inOuttakeRelease;

    public double VOLTAGE, driveTrainSpeed, maxDriveTrainSpeed, Y_SPEED, X_SPEED, H_SPEED, lockedAngle;

    public ActionPress outtakeCloseClaw, intakeCloseClaw, intakePickUp, switchOuttakeStateAction, modeSwitch, transferAction, p2pAction, lowBasketAction, driveMode;
...
//        cameraDetection = new CameraDetection(hardwareMap, telemetry, true, Side);
        ...
        if (!isAuto){
            ...
            driveMode = new ActionPress(this::switchDriveMode);
    ...
    public void teleOpControl(){
        ...
        outtakeCloseClaw.update((g1.right_trigger > 0));
        intakeCloseClaw.update((g1.left_trigger > 0));

        if (DRIVEMODE){
            switchOuttakeStateAction.update(g1.right_bumper, outtake.canChangeSlidePoseTimed());
        }else{
            intakePickUp.update(g1.right_bumper, intake.isArmPose(IntakeArmPose.DETECTION) && !(inTransfer1 || inTransfer2));
        }

        transferAction.update(g1.b, intake.isArmPose(IntakeArmPose.DETECTION) && !DRIVEMODE);

//        lowBasketAction.update(g1.b, outtake.canChangeSlidePoseTimed());
        modeSwitch.update(g1.dpad_up, outtake.isSlidePose(OuttakeSlidePose.INITIAL));
        driveMode.update(g1.a);
...
//        }
        ...
        if (inOuttakeRelease && releaseOuttakeTimer.getElapsedTimeSeconds() > 0.3){
            outtake.setSlidePose(OuttakeSlidePose.INITIAL);
            outtake.setOuttakeArmPose(OuttakeArmPose.TRANSFER);
            inOuttakeRelease = false;
        }

        //IntakeSlideControl
        double slideSpeed = 0.4;
        if (g1.y) slideSpeed = 1;

        if (intake.isSlidePose(IntakeSlidePose.FREE) && !DRIVEMODE) intake.slideControl(g1.left_stick_y * slideSpeed);
        else if (!intake.magSwitch.isPressed()) intake.slideControl(-0.3);
...
//        int Y_DIR = (g1.dpad_left ? -1 : 0) + (g1.dpad_right ? 1 : 0);
        ...
        Y_SPEED = -Math.min(Math.pow(Math.abs(g1.left_stick_x), 3) * driveTrainSpeed, maxDriveTrainSpeed) * 0.5;
//        Y_SPEED = maxDriveTrainSpeed * 0.5;
    ...
    public void teleOpDebug(){
        ...
        this.t.addData("DRIVEMODE", this.DRIVEMODE);
    }
    ...
    public void switchOuttakeState(){
        if (outtake.isSlidePose(OuttakeSlidePose.INITIAL)){
            ...
            if (MODE){
                releaseOuttakeTimer.resetTimer();
                inOuttakeRelease = true;
...
//        cameraDetection.update();
        ...
        if (modeSwitchTimer.getElapsedTimeSeconds() > 1 && switching){
            outtake.setSlidePose(OuttakeSlidePose.INITIAL);
            switching = false;
        }
